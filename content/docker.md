...
* [Docker compose](#Docker_compose)
  ...
  + [Файл `compose.yml`](#Docker_compose-Файл_сompose_yml)

  + [Команды](#Docker_compose-Команды)

  + [Пример работы](#Docker_compose-Пример_работы)

  + [Ссылки](#Docker_compose-Ссылки)
...
[`docker container attach CONTAINER`](https://docs.docker.com/reference/cli/docker/container/attach/)

: Прикрепить стандартный ввод (`stdin`), вывод (`stdout`) и вывод ошибок
  (`stderr`) к запущенному контейнеру

  Для остановки контейнера необходимо использовать комбинацию `CTRL-c`. Данная
  комбинация пошлет сигнал `SIGKILL` контейнеру. Если установлена опция
  `--sig-proxy` (по умолчанию она установлена), то `CTRL-c` пошлет `SIGINT`
  контейнеру. Если контейнер был запущен с опциями `-i|--interactive` и
  `-t|--tty` можно отключиться от контейнера и при этом не останавливая его с
  помощью комбинации `CTRL-p`, `CTRL-q`.
...
: Создать сеть
  ...
  1. Создать сеть с указанным именем (`sample`);
...
[`docker container port CONTAINER`](https://docs.docker.com/reference/cli/docker/container/port/)

: Вывести пары связанных портов указанного контейнера и хоста
...
## Пример работы ##
{: id="Docker_compose-Пример_работы" }

1. `docker compose up {-d|--detach} --pull never`

   : * Собрать контейнеры

     * Не подключаться к запущенным контейнерам (`-d`)

     * Не скачивать образы из репозитория (`--pull never`)

2. `docker compose logs {-f|--follow} [SERVICE]`

   : Вывести журнал для указанного сервиса (`SERVICE`)/всех сервисов

3. `docker container exec {-i|--interactive} {-t|--tty} [CONTAINER] sh`

   : Создать интерактивную (`-i`) сессию командной оболочки (`sh`) в выделенном
     терминале (`-t`) в указанном контейнере (`CONTAINER`)

     Получив доступ к терминалу запущенного контейнера, можно осуществлять работу
     с запущенной программой в отладчике. Для этого необходимо запустить
     отладчик следующей командой: `gdb --pid 1 EXECUTABLE`.
...
4. `docker compose down --rmi all`
   ...
   : * Остановить запущенные контейнеры
     ...
     * Удалить созданные образы (`--rmi all`)
