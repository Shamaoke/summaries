...
* [Очистка репозитория](#Очистка_репозитория)
...
: снимок (snapshot) состояния репозитория

  связанная группа правок

  действие по сохранению (фиксации) снимка состояния репозитория

to commit

: сохранить (зафиксировать) снимок состояния репозитория

commit message

: комментарий к сохранению снимка

  _сообщение, поясняющее дейстивие по сохранению снимка_

staged area

: область подготовленных правок
{: id="staged_area"}

unstaged area

: область неподготовленных правок
...
: отменить подготовленную правку

tracked file

: отслеживаемый файл

  _файл, находящийся под версионным контролем_
{: id="tracked_file"}
...
: неотслеживаемый файл
  ...
  _файл, который не находится под версионным контролем_
{: id="untracked_file"}
...
тематическая ветвь (_topic branch_)
...
: Изменить системный параметр
  ...
  _Изменения будут сохранены в файле `/etc/gitconfig`._

`git config OPTION VALUE`

: Изменить локальный параметр

  _Изменения будут сохранены в файле `.git/config`._
...
: Создать замену для стандартной команды

`core.editor EDITOR`

: Установить используемый текстовый редактор

`core.pager PAGER`

: Установить используемый пейджер

`core.quotepath { true | false }`

: Разрешить/запретить вывод имен файлов, содержащих символы UTF-8 большие 0x80, в виде числовых кодов при выполнении некоторых команд

`merge.tool MERGE_TOOL`

: Установить используемый инструмент слияния файлов

`diff.tool DIFF_TOOL`

: Установить используемый инструмент сравнения файлов

`difftool.prompt { true | false }`

: Разрешить/запретить выдачу запроса на подтверждение каждый раз при запуске инструмента сравнения файлов

`commit.template TEMPLATE`

: Задать шаблон для комментария

`core.excludesfile PATH`

: Задать путь к внешнему файлу с исключениями

`help.autocorrect DELAY_IN_DECISECONDS`

: Установить автоматическое исправление опечаток в командах
...
: Открыть интерактивное меню выбора правок для добавления их в индекс
...
: Создать ветвь `NAME` из отложенной правки `N` и удалить последнюю
...
# Очистка репозитория #
{: id="Очистка_репозитория" }

`git clean`

: Удалить все неотслеживаемые файлы из истории

`git clean -f|--force -d`

: Удалить все неотслеживаемые файлы из истории

  Использовать принудительный режим удаления (`--force`)

  Удалить неотслеживаемые каталоги и их содержимое (`-d`)
...
`git branch {-D|--delete --force} NAME`

: Удалить ветвь вне зависимости от того, слита она или нет

`git branch`

: Показать все текущие ветви и отметить рабочую

`git branch -v`

: Показать все ветви, отметить текущую и вывести последнюю передачу для каждой ветви

`git branch -r`

: Показать все ветви в удаленном репозитории
...
: Восстановить ветвь с указанной метки

`git branch -d NAME`

: Удалить ветвь

  Ветвь должна быть слита.

`git branch -rd NAME`

: Удалить ветвь в удаленном репозитории
...
: Добавить в удаленный репозитори указанную метку

`git push --tags`

: Добавить в удаленный репозиторий все имеющиеся метки

`git push --delete origin TAG`

: Удалить метку в удаленном репозитории

`git push origin --delete $(git tag --list)`

: Удалить все метки из удаленного репозитория
...
{: id="Работа_с_удаленными_репозиториями"}
    ...
    git push [<имя_удаленного_репозитория>] - обновить удаленный репозиторий.
      ...
      git request-pull <начальная_ссылка> <url_репозитория> [<конечная_ссылка>] - вывести на экран сводную статистику по передачам, ожидающим вытягивания со стороны менеджера.
